...
#define DEBUG
#define LOG_FLAG	"sipa_timer_task"
...
#define MAX_PROC_INFO_NAME_LEN			(64)
#define MAX_TIMER_TASK_NUM				(SIPA_TIMER_TASK_INVALID_HDL)
#define MAX_TIMER_PROCESSOR_NUM			(16)
#define TIMER_TASK_CYCLE_TIME_MS		(100)

#define INVALID_USER_ID					(0xFFFFFFFF)

typedef enum sipa_task_state_e {
	SIA81XX_TASK_DISABLE = 0,
	SIA81XX_TASK_ENABLE,
} sipa_task_state_t;

typedef struct sipa_timer_proc_info_s {
	...
	int onlyonce;
    int first_trigger;
} sipa_timer_proc_info_t;

typedef struct sipa_timer_task_s {
	atomic_t task_state;
	atomic_t switch_flag;
	struct task_struct *task;
	struct mutex task_mutex;

	sipa_timer_proc_info_t proc[MAX_TIMER_PROCESSOR_NUM];//must be a array
} sipa_timer_task_t;

static sipa_timer_task_t timer_tasks[MAX_TIMER_TASK_NUM];

static inline void sipa_process_one_timer_callback(
	sipa_timer_proc_info_t *proc, uint32_t time, int is_first)
{
	int ret = 0;
	...
	if (proc->onlyonce) {
		memset(proc, 0, sizeof(sipa_timer_proc_info_t));
		proc->user_id = INVALID_USER_ID;
	}
...
static int sipa_timer_task_processor(
	void *data)
{
	static unsigned int cur_run_cnt;//only for debug
	int i = 0;
	uint32_t switch_flag = 0;
	sipa_timer_task_t *timer = data;
	if (NULL == timer) {
	...
	while (SIA81XX_TASK_ENABLE == atomic_read(&timer->task_state)) {
		...
		for (i = 0; i < ARRAY_SIZE(timer->proc); i++) {
			...
			if ((1 == switch_flag) && (1 == timer->proc[i].first_trigger)) {
			...
			sipa_process_one_timer_callback(
				&timer->proc[i],
...
static void sipa_timer_task_record_proc_info(
	sipa_timer_proc_info_t *proc,
	const char *name,
	uint32_t user_id,
	uint32_t wake_up_interval_ms,
	int (*process)(int is_first, void *data),
	...
	int first_trigger)
{
	...
	proc->run_time_ms = 0;
	proc->onlyonce = OnlyOnce;
	proc->first_trigger = first_trigger;
...

{
	...
	for (i = 0; i < ARRAY_SIZE(timer->proc); i++) {
		if (INVALID_USER_ID == timer->proc[i].user_id) {
			sipa_timer_task_record_proc_info(
				&timer->proc[i],
				name,
				user_id,
				wake_up_interval_ms,
	...
	uint32_t hdl)
{
	...
	timer->task = kthread_create(
		sipa_timer_task_processor, timer, "sia81xx_timer_task_processor");
