...
#define CAL_PACKET_LEN(packet) \
	((packet)->payload_size + sizeof(struct sipa_cal_packet))
	...
	int max_len)
{
	...
	case OPT_SET_CAL_VAL:
	{
		...
		if (NULL == map) {
			...
			ret = sipa_sock_write(sock, buf, len);
...
static int sipa_sock_server_task(
	void *data)
{
	int ret = 0;
...
static int sipa_open_sock_task(void)
{
	...
	socket_task = kthread_create(
		sipa_sock_server_task, NULL, "sipa_socket");
	if (NULL == socket_task) {
...
static int sipa_close_sock_task(void)
{
	...
	while (TASK_STATE_CLOSED != sock_task_state) {
		...
		if (0 >= max_wait_time_ms) {
			pr_err("[  err][%s] %s: sipa_wait_sock_task_close "
				"time out\r\n",
				LOG_FLAG, __func__);
			return -EBUSY;
		}
...
//EXPORT_SYMBOL(sipa_close_sock_server);
