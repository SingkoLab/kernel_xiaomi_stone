...
#define DEBUG
#define LOG_FLAG	"sia91xx_common"
	...
	struct snd_soc_dai *dai)
{
	...
	pr_info("[ info][%s] %s: i2s rate: %u !!! \r\n", 
		LOG_FLAG, __func__, rate);
	...
	struct snd_soc_dai *dai)
{
...
#if 0
	sipa_param_read_spk_calibration(si_pa->channel_num, &cal_spk);
	if (cal_spk.cal_ok  == SIPA_CAL_SPK_FAIL) {
		sipa_auto_first_cal_r0(
	...
	if (1 == si_pa->spk_model_flag) {
		sipa_auto_set_spk_model(
				si_pa->timer_task_hdl,
				si_pa->dyn_ud_vdd_port,
				si_pa->channel_num);

		sipa_auto_get_spk_model(
				si_pa->timer_task_hdl,
				si_pa->dyn_ud_vdd_port,
				si_pa->channel_num);
	}
...
int sia91xx_codec_probe(struct snd_soc_codec *codec)
{
	...
	si_pa->sia91xx_wq = create_singlethread_workqueue("sia91xx");
	if (!si_pa->sia91xx_wq)
		return -ENOMEM;
