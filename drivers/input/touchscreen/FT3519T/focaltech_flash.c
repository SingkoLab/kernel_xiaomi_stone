...
/*read lockdown_info from flash start*/
#if FOCAL_LOCKDOWN
int lct_get_lockdown_info(void)
{
	...
	strlcpy(fts_data->fts_lockdowninfo, tp_lockdown_info_buf, 64);

	return 0;
}
#endif
/*read lockdown_info from flash end*/
...
{
	fts_read_reg(FTS_REG_FW_VER, &fw_version);
	seq_printf(m, "[Vendor]Samsung [TP-IC]: FT3519T [FW]0x%x\n", fw_version);
	return 0;
}
