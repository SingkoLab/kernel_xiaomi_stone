...
#ifdef FTS_RAW_DIFF_EN
#define FTS_PROC_RAWDATA "fts_tp_rawdata"
#define FTS_PROC_DIFFER "fts_tp_differ"
struct lct_fts_raw_diff{
	u8 tx_num;
	u8 rx_num;
	int *rawdata;
	int *diffdata;
	struct proc_dir_entry *fts_tp_rawdata_proc;
	struct proc_dir_entry *fts_tp_differ_proc;
};
static struct lct_fts_raw_diff *lct_fts_raw_diff_p;
#endif
...
int wait_state_update(u8 retval)
{
	...
	if ((IS_ERR_OR_NULL(tdata)) || (IS_ERR_OR_NULL(tdata->func))) {
...
static int fts_test_save_data_csv_private(struct fts_test *tdata)
{
	int ret = 0;
#if CSV_SUPPORT
	char *csv_buffer = NULL;
...
static int fts_test_main_exit(void)
{
	...
	FTS_TEST_FUNC_ENTER();
	if (tdata->func && tdata->func->save_data_private)
		ret = fts_test_save_data_csv_private(tdata);
	else
		ret = fts_test_save_data_csv(tdata);
	ret1 = fts_test_save_result_txt(tdata);
	if ((ret == 0) && (ret1 == 0))
		ito_test_status = 3;
...
static int fts_ito_test_show(struct seq_file *file, void *data)
{
	...
	for (i = FTS_ENTER_FACTORY_MODE ; i <= FTS_SHORT_TEST;  i++)
	...
	//Rawdata result && Scap cb test result
	FTS_INFO("fts_ftest->test_item[FTS_RAWDATA_TEST].testresult = %d", fts_ftest->test_item[FTS_RAWDATA_TEST].testresult);
	FTS_INFO("fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult = %d", fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult);
	FTS_INFO("fts_ftest->test_item[FTS_RAWDATA_UNIFORMITY_TEST].testresult = %d", fts_ftest->test_item[FTS_RAWDATA_UNIFORMITY_TEST].testresult);
	FTS_INFO("fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult = %d", fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult);
	FTS_INFO("fts_ftest->test_item[FTS_SHORT_TEST].testresult = %d", fts_ftest->test_item[FTS_SHORT_TEST].testresult);
	FTS_INFO("fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult = %d", fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult);
	if ((fts_ftest->test_item[FTS_RAWDATA_TEST].testresult) && (fts_ftest->test_item[FTS_SCAP_CB_TEST].testresult)
		 && (fts_ftest->test_item[FTS_RAWDATA_UNIFORMITY_TEST].testresult))
		test_result_bmp[RAWDATA] = 'P';
	else
		test_result_bmp[RAWDATA] = 'F';

	//Scap rawdata test result
	if ((fts_ftest->test_item[FTS_SCAP_RAWDATA_TEST].testresult))
		test_result_bmp[SCAP_RAWDATA_DATA] = 'P';
	else
		test_result_bmp[SCAP_RAWDATA_DATA] = 'F';
	//short test result
	if ((fts_ftest->test_item[FTS_SHORT_TEST].testresult))
		test_result_bmp[SHORT_DATA] = 'P';
	else
		test_result_bmp[SHORT_DATA] = 'F';
	//panel differ test result
	if ((fts_ftest->test_item[FTS_PANEL_DIFFER_TEST].testresult))
		test_result_bmp[PANEL_DIFFER_DATA] = 'P';
	else
		test_result_bmp[PANEL_DIFFER_DATA] = 'F';
	seq_printf(file, "0%c-1%c-2%c-3%c-4%c\n",
		test_result_bmp[I2C_COMMUNICATION],
		test_result_bmp[RAWDATA],
		test_result_bmp[SCAP_RAWDATA_DATA],
		test_result_bmp[SHORT_DATA],
		test_result_bmp[PANEL_DIFFER_DATA]);
...
#ifdef FTS_RAW_DIFF_EN
static int lct_tp_get_diff(int *data, int byte_num)
{
	int ret = 0;
	u8 old_mode = 0;
	struct fts_test *tdata = fts_ftest;
	...
	/* read diffdata */
	ret = read_rawdata(tdata, 0x01, 0xAD, FACTORY_REG_RAWDATA_ADDR_MC_SC, byte_num, data);
	if (ret < 0) {
...
static int lct_tp_get_raw(int *data, int byte_num)
{
	int ret = 0;
	struct fts_test *tdata = fts_ftest;
...
static int fts_diff_proc_show(struct seq_file *file, void *data)
{
	int row = 0, col = 0;
	u8 tx_num = lct_fts_raw_diff_p->tx_num;
	u8 rx_num = lct_fts_raw_diff_p->rx_num;
	int *diffdata = lct_fts_raw_diff_p->diffdata;
	...
	if (file->count < file->size) {
		if (!IS_ERR_OR_NULL(lct_fts_raw_diff_p->diffdata))
			kfree(lct_fts_raw_diff_p->diffdata);
		lct_fts_raw_diff_p->diffdata = NULL;
	}
	return 0;
}
static int fts_raw_proc_show(struct seq_file *file, void *data)
{
	int row = 0, col = 0;
	u8 tx_num = lct_fts_raw_diff_p->tx_num;
	u8 rx_num = lct_fts_raw_diff_p->rx_num;
	int *rawdata = lct_fts_raw_diff_p->rawdata;
	...
	//FTS_TEST_ERROR("file->size = %lu, file->count = %lu", file->size, file->count);
	if (file->count < file->size) {
		if (!IS_ERR_OR_NULL(lct_fts_raw_diff_p->rawdata))
			kfree(lct_fts_raw_diff_p->rawdata);
		lct_fts_raw_diff_p->rawdata = NULL;
	}
	return 0;
}
static int fts_tp_differ_proc_open (struct inode *inode, struct file *file)
{
	int ret = 0;
	u8 tx_num = 0;
	u8 rx_num = 0;
	...
	if (lct_fts_raw_diff_p->diffdata) {
	...
	lct_fts_raw_diff_p->tx_num = tx_num;
	lct_fts_raw_diff_p->rx_num = rx_num;

	//malloc buffer
	lct_fts_raw_diff_p->diffdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p->diffdata)) {
	...
	//get diffdata
	ret = lct_tp_get_diff(lct_fts_raw_diff_p->diffdata, tx_num * rx_num * 2);
	if (ret < 0) {
		FTS_TEST_ERROR("lct_tp_get_diff() failed, ret = %d", ret);
...
err_disable_clb_fail:
	FTS_TEST_FUNC_EXIT();
	return single_open(file, fts_diff_proc_show, NULL);
}

static int fts_tp_rawdata_proc_open (struct inode *inode, struct file *file)
{
	int ret = 0;
	u8 tx_num = 0;
	u8 rx_num = 0;
	...
	//malloc buffer
	lct_fts_raw_diff_p->rawdata = (int *)kzalloc(tx_num * rx_num * sizeof(int), GFP_KERNEL);
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p->rawdata)) {
	...
	//get rawdata
	ret = lct_tp_get_raw(lct_fts_raw_diff_p->rawdata, tx_num * rx_num * 2);
	if (ret < 0) {
		FTS_TEST_ERROR("lct_tp_get_raw() failed, ret = %d", ret);
		goto err_get_data_fail;
	}
	goto exit;
...
err_disable_clb_fail:
	FTS_TEST_FUNC_EXIT();
	return single_open(file, fts_raw_proc_show, NULL);
}

static const struct file_operations fts_tp_rawdata_proc_fops = {
	.owner = THIS_MODULE,
	.open = fts_tp_rawdata_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,

};
static const struct file_operations fts_tp_differ_proc_fops = {
	.owner = THIS_MODULE,
	.open =  fts_tp_differ_proc_open,
	.read = seq_read,
	.llseek = seq_lseek,
	.release = single_release,

};
static int fts_fts_raw_diff_proc_init(void)
{
	int ret = 0;

	FTS_TEST_FUNC_ENTER();
	lct_fts_raw_diff_p = (struct lct_fts_raw_diff *)kzalloc(sizeof(struct lct_fts_raw_diff), GFP_KERNEL);
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p)) {
		FTS_TEST_ERROR("malloc lct_fts_raw_diff memory fail");
		//lct_fts_raw_diff_p = NULL;
		ret = -ENOMEM;
		goto err_malloc_fail;

	}
	lct_fts_raw_diff_p->fts_tp_rawdata_proc = proc_create(FTS_PROC_RAWDATA, 0444, NULL, &fts_tp_rawdata_proc_fops);
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p->fts_tp_rawdata_proc)) {
		FTS_TEST_ERROR("ERROR: create /proc/%s failed.", FTS_PROC_RAWDATA);
		lct_fts_raw_diff_p->fts_tp_rawdata_proc = NULL;
		ret = -1;
		goto err_create_procfs_fail;

	}
	lct_fts_raw_diff_p->fts_tp_differ_proc = proc_create(FTS_PROC_DIFFER, 0444, NULL, &fts_tp_differ_proc_fops);
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p->fts_tp_differ_proc)) {
		FTS_TEST_ERROR("ERROR: create /proc/%s failed.", FTS_PROC_DIFFER);
		//lct_fts_raw_diff_p->fts_tp_differ_proc = NULL;
		ret = -1;
		goto err_create_procfs_fail;

	}

	FTS_INFO("create /proc/%s", FTS_PROC_RAWDATA);
	FTS_INFO("create /proc/%s", FTS_PROC_DIFFER);

	return 0;

err_create_procfs_fail:
	if (lct_fts_raw_diff_p) {
		kfree(lct_fts_raw_diff_p->fts_tp_differ_proc);
		lct_fts_raw_diff_p->fts_tp_differ_proc = NULL;
		kfree(lct_fts_raw_diff_p);
		lct_fts_raw_diff_p = NULL;
	}
err_malloc_fail:
	FTS_TEST_FUNC_EXIT();
...
static void fts_fts_raw_diff_proc_exit(void)
{
	if (IS_ERR_OR_NULL(lct_fts_raw_diff_p))
		return;
	if (!IS_ERR_OR_NULL(lct_fts_raw_diff_p->fts_tp_rawdata_proc)) {
		remove_proc_entry(FTS_PROC_RAWDATA, NULL);
		FTS_INFO("remove /proc/%s", FTS_PROC_RAWDATA);
	}
	if (!IS_ERR_OR_NULL(lct_fts_raw_diff_p->fts_tp_differ_proc)) {
		remove_proc_entry(FTS_PROC_DIFFER, NULL);
		FTS_INFO("remove /proc/%s", FTS_PROC_DIFFER);
	}
	kfree(lct_fts_raw_diff_p);

}
#endif
/*creat proc/tp_data_dump to show the tx and rx start*/
#ifdef FTS_TP_DATA_DUMP_EN
...
err_create_procfs_fail:
	if (lct_tp_data_dump_p) {
		if (lct_tp_data_dump_p->tp_data_dump_proc) {
			kfree(lct_tp_data_dump_p->tp_data_dump_proc);
			lct_tp_data_dump_p->tp_data_dump_proc = NULL;
		}
...
#endif
/*creat proc/tp_data_dump to show the tx and rx end*/
	...
	size_t count, loff_t *pos)
{
	int ret = 0;
	char tmp[6] = {0};
	char fwname[FILE_NAME_LENGTH] = "focaltech_mp_fw_sdc.ini";
	struct fts_ts_data *ts_data = fts_data;
